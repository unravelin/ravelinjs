...
export async function updateCommitStatus(logs) {
  try {
    ...
    // Read results.json file generated by Mocha
    const counts = {};
    try {
      const resultsData = await fs.readFile('results.json', 'utf8');
      const results = JSON.parse(resultsData);

      console.log(results);
      counts.passed = results.stats.passes || 0;
      counts.failed = results.stats.failures || 0;
    } catch (err) {
      console.error('Error reading results.json file:', err);
      return;
    }
