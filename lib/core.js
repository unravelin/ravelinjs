...
export function Core(cfg) {
  ...
  this.ids = this._ids();
  this.id = this.ids.then(function(ids) {
    return ids.device;
  });
  if (cfg.attach !== false) {
    this.attach();
 ...
 */
Core.prototype.attach = function() {
  var c = this.cookies;
...
/**
 * _ids reads our device identity from the browser.
 * @returns {Promise<IDs>}
 */
Core.prototype._ids = function() {
  var c = this.cookies;
  return new this.Promise(function(resolve) {
    var ids = {
      device: c.get('ravelinDeviceId') || uuid(),
      session: c.get('ravelinSessionId') || uuid()
    };
    c.set('ravelinDeviceId', ids.device);
    c.set('ravelinSessionId', ids.session);
    resolve(ids);
  });
};
