...
import { bind, promiseRetry, uuid } from './util';
 ...
 * @prop {string} [prefix=rjs-] A string of the default prefix for the deviceId.
...
export function Core(cfg) {
  ...
  this._id = cfg.id && typeof(cfg.id) === 'function' ? cfg.id : Promise.resolve(cfg.id);
  this.sync();
  if (cfg.init !== false) {
