import enc from '../src/encryption-vendored';
var sjcl = enc.sjcl;
var RSAKey = enc.RSAKey;
...
/**
 * encrypt a card into an encrypted payment method that can be set via your
 * server to Ravelin.
 *
 * @param {Card} card
 * @return {EncryptedCard}
 */
export default function encrypt(card) {
  if (!card) {
  ...
  if (!(card.year > 2000)) {
    throw new Error('raveiln/encrypt: year should be in the 21st century');
  }
  ...
  // base64.
  var aesResult = aesEncrypt(JSON.stringify({
    pan: card.pan,
  ...
  var rsaResultB64 = rsaEncrypt(key, aesResult.aesKeyB64, aesResult.ivB64);
  ...
  return {
    ...
    ravelinSDKVersion: process.env.npm_package_version,
    keyIndex: key.index,
