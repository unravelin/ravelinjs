...
export function promiseRetry(P, factory, retries, retryBackoffMs) {
  return new P(function(resolve, reject) {
    ...
    function attempt(retries, delay) {
      return new P(function(r) { setTimeout(r, delay); })
        .then(function() {
          return factory(function retry() {
            throw new Error('promise-retry');
          }).then(
            resolve,
            function(e) {
              if (e && e.message === 'promise-retry' && retries > 0) {
                return attempt(retries - 1, delay + (retryBackoffMs || 150));
              }
              reject(e);
            }
          );
