  ...
  return Promise.all(promises).then(function(result) {
    ...
    return that.core.send('POST', '/z', {events: [{
      ...
      eventMeta: {
        ...
        suspectedBot: botDetected.bot ? botDetected.botKind : botDetected.bot,
...
Track.prototype.botDetected = function() {
  if (!this._botDetector) {
    this._botDetector = loadBotd({ monitoring: false });
  }

  return this._botDetector.then(botd => botd.detect())
    .then((result) => result)
