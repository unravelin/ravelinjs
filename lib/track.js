...
import { promiseAll, uuid } from "./util";
  ...
  return promiseAll(this.core.Promise, promises).then(function(result) {
    ...
    return that.core.send('POST', '/z', {events: [{
      ...
      eventMeta: {
        ...
        agentDetected: {
          isAgent: agentDetected.isAgent,
          confidence: agentDetected.confidence
        }
      }
    }]});
  });
 ...
 * @returns {Promise<boolean>}
 */
Track.prototype.agentDetected = function() {
  if (!this._agentDetector) {
    this._agentDetector = agentDetector.init({ debug: false, autoStart: true });
  }

  return this._agentDetector.finalizeDetection().then(res => {
    console.log(res);
