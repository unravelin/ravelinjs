...
Track.prototype._attach = function() {
  ...
  if (document.addEventListener) {
    document.addEventListener('paste', bind(this.paste, this));
  }
};
...
Track.prototype.paste = function(e) {
  ...
  if (t) {
    // Locate the field.
    if (t.name) {
      props.fieldName = t.name;
    }
    if (t.form) {
      var form = t.form;
      props.formName = attr(form, 'name') || attr(form, 'id');
      props.formAction = attr(form, 'action');
    }
