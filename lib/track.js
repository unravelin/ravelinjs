 ...
 * @param {string} type Identifies the origin of the event.
 * @param {string} name A description of what occurred.
 * @param {object} [props]
 */
Track.prototype._send = function(type, name, props) {
  var that = this;
  var promises = [this.core.ids(), this.incognitoDetected(), this.botDetected()];
  return Promise.all(promises).then(function(result) {
    var ids = result[0];
    var incognitoDetected = result[1];
    var botDetected = result[2];
...
Track.prototype.incognitoDetected = function() {
  ...
  this._incognitoDetected = detectIncognito().then(res => res.isPrivate).catch(() => false);

  return this._incognitoDetected;
};

/**
 * botDetected returns a promise-wrapped object indicating if a track event
 * was triggered by a suspected bot.
 *
 * @returns {Promise<object>}
 */
Track.prototype.botDetected = function() {
  if (this._botDetected) {
    return this._botDetected;
  }

  this._botDetected = loadBotd({ monitoring: false }).then(botd => botd.detect()).catch(() => ({ bot: false }));
...
/**
 * load event handler.
 * @param {object} [props] Additional properties describing the page loaded.
 */
Track.prototype.load = function(props) {
 ...
 * @param {object} [props] Additional properties to describe the event.
 * @returns
 */
Track.prototype.event = function(name, props) {
...
Track.prototype.paste = function(e) {
  ...
  if (t) {
    ...
    var custom = this.classifyPaste && this.classifyPaste(e);
    ...
    sensitive = t.type == 'password' || treeHasAttr(t, 'data-rvn-sensitive') || (custom && custom.sensitive);
    if (!sensitive) {
      if (t.value) {
